...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'Model "{model_name}" not found in user'))
            return
    ...
    def generate_views(self, model):
        ...
        views_content = [
            "",
            f"class {model_name}ListView(PermissionRequiredMixin, LoginRequiredMixin,PageHeaderMixin, CustomSingleTableMixin,  FilterView):",
            f"    model = {model_name}",
            f"    table_class = {model_name}Table",
            f"    template_name = 'list.html'",
            f"    permission_required = 'user.view_{model_name.lower()}'",
            f"    filterset_class = {model_name}Filter",
            f"    page_title = 'All {model_name}s'",
            f"    add_link = reverse_lazy('{model_name.lower()}_add')",
            f"    add_perms = 'user.add_{model_name.lower()}'",
            f"    edit_perms = 'user.change_{model_name.lower()}'",
            f"    delete_perms = 'user.delete_{model_name.lower()}'",
            "",
            f"class {model_name}CreateView(PermissionRequiredMixin,PageHeaderMixin, LoginRequiredMixin, CreateView):",
            f"    model = {model_name}",
            f"    form_class = {model_name}Form",
            f"    template_name = 'add.html'",
            f"    permission_required = 'user.add_{model_name.lower()}'",
            f"    success_url = reverse_lazy('{model_name.lower()}_list')",
            f"    page_title = '{model_name}'",
            f"    list_link = reverse_lazy('{model_name.lower()}_list')",
            "",
            f"class {model_name}UpdateView(PermissionRequiredMixin,PageHeaderMixin, LoginRequiredMixin, UpdateView):",
            f"    model = {model_name}",
            f"    form_class = {model_name}Form",
            f"    template_name = 'add.html'",
            f"    permission_required = 'user.change_{model_name.lower()}'",
            f"    success_url = reverse_lazy('{model_name.lower()}_list')",
            f"    page_title = 'Update {model_name}'",
            f"    list_link = reverse_lazy('{model_name.lower()}_list')",
            "",
            f"class {model_name}DeleteView(PermissionRequiredMixin,PageHeaderMixin, LoginRequiredMixin, DeleteView):",
            f"    model = {model_name}",
            f"    template_name = 'delete.html'",
            f"    permission_required = 'user.delete_{model_name.lower()}'",
    ...
    def generate_forms(self, model):
        ...
        # Write the content to the forms file
        with open(forms_file, 'a') as file:
            file.write("\n".join(forms_content) + "\n")

    def generate_filters(self, model):
        model_name = model.__name__
        filters_file = "apps/user/filters.py"
        ...
        # Write the content to the filters file
        with open(filters_file, 'a') as file:
            file.write("\n".join(filters_content) + "\n")

    def generate_tables(self, model):
        model_name = model.__name__
        tables_file = "apps/user/tables.py"

        # Add imports if missing
        add_import_if_missing(tables_file, 'from apps.helpers import CustomTable')
    ...
    def generate_urls(self, model):
        ...
        # Find urlpatterns and add the new URL patterns if they don't exist already
        urlpatterns_index = None
        for index, line in enumerate(content):
            if 'urlpatterns' in line:
                urlpatterns_index = index
                break

        if urlpatterns_index is not None:
            # Insert URL patterns before the closing bracket of urlpatterns
            for pattern in url_patterns:
                if pattern.strip() not in "".join(content):  # Avoid adding duplicates
                    content.insert(urlpatterns_index + 1, f"{pattern}\n")
